...
function man_z_calib_definition() {

    defineRule(config.MAN_PREFIX + "_z_calib" + CALIB_STEPS.STANDBY, {
        when: function() {
            return dev["man"]["current_rule"] == config.MAN_RULES.AXIS_Z_CALIB &&
                dev["man"]["current_step"] == CALIB_STEPS.STANDBY;
        },
        then: function() {
            if (drivers.MAN_MOTOR_DRIVER_OBJ.check_sensor_state(config.MAN_Z_PREFIX, dev[config.MAN_PREFIX]["axis_z_limit_sensor"])) {
    ...
    defineRule(config.MAN_PREFIX + "_z_calib" + CALIB_STEPS.RUNNING, {
        when: function() {
            return dev["man"]["current_rule"] == config.MAN_RULES.AXIS_Z_CALIB &&
                dev["man"]["current_step"] == CALIB_STEPS.RUNNING;
        },
        then: function() {
            MAN_MODEL_FUNC.axis_z_motor_speed_control_request(dev[config.MAN_PREFIX]["axis_z_default_calib_reverse"], dev[config.MAN_PREFIX]["axis_z_default_calib_speed_high"], dev[config.MAN_PREFIX]["axis_z_default_calib_acc"]);
    ...
    defineRule(config.MAN_PREFIX + "_z_calib" + CALIB_STEPS.CHECK_SENSOR, {
        when: function() {
            return dev["man"]["current_rule"] == config.MAN_RULES.AXIS_Z_CALIB &&
                dev["man"]["current_step"] == CALIB_STEPS.CHECK_SENSOR &&
                drivers.MAN_MOTOR_DRIVER_OBJ.check_sensor_state(config.MAN_Z_PREFIX, dev[config.MAN_PREFIX]["axis_z_limit_sensor"]);
        },
        then: function() {
    ...
    defineRule(config.MAN_PREFIX + "_z_calib" + CALIB_STEPS.RUNNING_BACK, {
        when: function() {
            return dev["man"]["current_rule"] == config.MAN_RULES.AXIS_Z_CALIB &&
                dev["man"]["current_step"] == CALIB_STEPS.RUNNING_BACK
        },
        then: function() {
            MAN_MODEL_FUNC.axis_z_motor_speed_control_request(!dev[config.MAN_PREFIX]["axis_z_default_calib_reverse"], dev[config.MAN_PREFIX]["axis_z_default_calib_speed_low"], dev[config.MAN_PREFIX]["axis_z_default_calib_acc"])
    ...
    defineRule(config.MAN_PREFIX + "_z_calib" + CALIB_STEPS.CHECK_SENSOR_BACK, {
        when: function() {
            return dev["man"]["current_rule"] == config.MAN_RULES.AXIS_Z_CALIB &&
        ...
        then: function() {
            drivers.MAN_MOTOR_DRIVER_OBJ.clear_position(config.MAN_Z_PREFIX);
            drivers.MAN_MOTOR_DRIVER_OBJ.emergency_stop(config.MAN_Z_PREFIX, dev[config.MAN_PREFIX]["axis_z_default_emergency_dec"])
        }
    });

    defineRule(config.MAN_PREFIX + "_z_calib_c" + CALIB_STEPS.CHECK_SENSOR_BACK, {
        when: function() {
            return dev["man"]["current_rule"] == config.MAN_RULES.AXIS_Z_CALIB &&
                dev["man"]["current_step"] == CALIB_STEPS.CHECK_SENSOR_BACK &&
                drivers.MAN_MOTOR_DRIVER_OBJ.check_running_completed(config.MAN_Z_PREFIX) &&
                !drivers.MAN_MOTOR_DRIVER_OBJ.check_sensor_state(config.MAN_Z_PREFIX, dev[config.MAN_PREFIX]["axis_z_limit_sensor"]);
        },
        then: function() {
